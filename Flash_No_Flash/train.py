...
parser.add_argument('--unet_depth', default=1, type=int,help='Display frequency by iteration count')
...
def camera_to_rgb(im,batch):
    return tfu.camera_to_rgb_jax(
      im/batch['alpha'], batch['color_matrix'], batch['adapt_matrix'])

def stencil_residual(pp_image, params, batch):
  avg_weight = (1. / 2.) ** 0.5 *  (1. / pp_image.reshape(-1).shape[0] ** 0.5)
  g = model(params,batch)  

#   g = camera_to_rgb(predicted, batch)
  noisy = camera_to_rgb(batch['noisy'], batch)
...
def linear_solver_id(d,x,hp_nn,data):
  ...
  d = linear_solve.solve_cg(matvec=Ax,
                          b=-jtf(x),
                          init=d,
                          maxiter=opts.nlin_iter,tol=1e-15)
...
def nonlinear_solver_id(init_image,hp_nn,data):
  ...
  def loop_body(args):
    d, linea_opt = linear_solver_id(None,args['x'],hp_nn,data)
    args['x'] += 1.0 * d

    args['linear_opt_err'] = args['linear_opt_err'].at[args['count'].astype(int)].set(linea_opt)
    args['gn_opt_err'] = args['gn_opt_err'].at[args['count'].astype(int)].set(optim_cond(args['x']))
    args['gn_loss'] = args['gn_loss'].at[args['count'].astype(int)].set(screen_poisson_objective(args['x'],hp_nn,data))
    args['count'] += 1
    return args

  loop_count = 3
  args = {'x':x,'count':jnp.array([0.0]),'gn_opt_err':-jnp.ones(loop_count),'gn_loss':-jnp.ones(loop_count),'linear_opt_err':-jnp.ones(loop_count)}
  ...
  for i in range(loop_count):
      args = loop_body(args)
#   args = jax.lax.fori_loop(0,loop_count,lambda i, val:loop_body(val),args) 
...
def visualize(x, params, batch):
    ...
    imshow = jnp.concatenate((xp,ambient,x_x,g_x,x_y,g_y),axis=-2)
    imshow = jnp.clip(jnp.abs(imshow),0,1)
    return imshow
