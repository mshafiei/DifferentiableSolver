...
def transformer(U, theta, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _interpolate(im, x, y, out_size, scale_h):
        with tf.compat.v1.variable_scope('_interpolate'):
            ...
            # compute interpolation weight before clipping
            ...
            # clip to only gather from valid pixels
    ...
    def _meshgrid(height, width, scale_h):
        with tf.compat.v1.variable_scope('_meshgrid'):
            ...
            if scale_h:
                ...
                x_t = tf.matmul(tf.ones(shape=tf.stack([height, 1])),
                                tf.transpose(tf.expand_dims(tf.linspace(0.0, tf.cast(width-1, 'float32'), width), 1), [1, 0]))
                y_t = tf.matmul(tf.expand_dims(tf.linspace(0.0, tf.cast(height-1, 'float32'), height), 1),
                                tf.ones(shape=tf.stack([1, width])))
    ...
    def _transform(theta, input_dim, out_size, scale_h):
        with tf.compat.v1.variable_scope('_transform'):
            ...
            smallers = 1e-6 * \
                (one - tf.cast(tf.greater_equal(tf.abs(t_s_flat), small), tf.float32))

            t_s_flat = t_s_flat + smallers
            condition = tf.reduce_sum(
                tf.cast(tf.greater(tf.abs(t_s_flat), small), tf.float32))
            # Ty changed
