...
def dim_image_jax(
        ambient,key, min_alpha=0.02, max_alpha=0.2, alpha=None):
    bsz = (ambient).shape[0]
    if alpha is None:
        alpha = jnp.power(10., jax.random.uniform(key,
            [bsz, 1, 1, 1], jnp.log10(min_alpha), jnp.log10(max_alpha)))
    return alpha * ambient, alpha
...
def add_read_shot_noise_jax(
    ...
    if sig_read is None or sig_shot is None:
        ...
        sig_read = jnp.power(10., jax.random.uniform(
            key1,[bsz, 1, 1, 1], minval=min_read, maxval=max_read))
        sig_shot = jnp.power(10., jax.random.uniform(
            key2,[bsz, 1, 1, 1], min_shot, max_shot))
    read = sig_read * jax.random.normal(key3,imgs.shape)
    shot = jnp.sqrt(imgs) * sig_shot * jax.random.normal(key4,imgs.shape)
    noisy = imgs + shot + read
    return noisy, sig_read, sig_shot
...
def estimate_std_jax(noisy, sig_read, sig_shot):
    return jnp.sqrt(sig_read**2 + jnp.maximum(0., noisy) * sig_shot**2)
...
def gamma_correct_jax(x):
    b = .0031308
    gamma = 1. / 2.4
    a = .055
    k0 = 12.92
    def gammafn(x): return (1 + a) * jnp.power(jnp.maximum(x, b), gamma) - a
    srgb = jnp.where(x < b, k0 * x, gammafn(x))
    k1 = (1 + a) * gamma
    srgb = jnp.where(x > 1, k1 * x - k1 + 1, srgb)
    return srgb
...
def camera_to_rgb_jax(imgs, color_matrix, adapt_matrix):
    ...
    xyz = jnp.linalg.solve(color_matrix, imgs)
    xyz = jnp.matmul(adapt_matrix, xyz)
    rgb = jnp.matmul(CONVERSION_MATRICES['xyz_to_rgb'][None,...], xyz)
    rgb = gamma_correct_jax(rgb)

    rgb = jnp.transpose(rgb, [0, 2, 1])
...
def get_psnr(pred, gt):
    ...
    psnr = tf.reduce_mean(-10. * tf.compat.v1.log(mse) / tf.compat.v1.log(10.))
    return psnr
