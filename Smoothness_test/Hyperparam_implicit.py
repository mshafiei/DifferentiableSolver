...
def main(args):
  ...
  def derivatives(x_star,z):
    dftdx = np.eye(len(x_star))
    dftdx[:-1,:-1] = -np.eye(len(x_star)-1)

    dftdl = np.zeros(len(x_star)+1)
    dftdl[1:-1] = x_star[1:] - x_star[:-1]
    dftdl[:-2] = x_star[:-1] - x_star[1:]
    
    dftdx = np.concatenate((dftdx,dftdl[:-1]),axis=0)

    dxdl = -np.matmul(np.invert(dftdx),dftdl)
    dfv = x_star - z
  ...
  dxy = np.stack((grad_val,np.ones_like(grad_val)),axis=-1)
  dxy_fd = np.stack((fd_val,np.ones_like(fd_val)),axis=-1)
  dxy = dxy / np.linalg.norm(dxy,axis=-1)[:,None] * -grad_val[:,None]
  dxy_fd = dxy_fd / np.linalg.norm(dxy_fd,axis=-1)[:,None] * -fd_val[:,None]
  plt.plot(grid,loss_val,'r')
  for i in range(grid.shape[0]):
    plt.arrow(grid[i],loss_val[i],dxy[i,1],dxy[i,0],color='purple')
    plt.arrow(grid[i],loss_val[i],dxy_fd[i,1],dxy_fd[i,0],color='lightgreen')
    
  plt.legend(['Validation loss','Analytic gradient','Finite difference'])
  ...
  #store the image
  for b in range(0,10): 
    outim = np.concatenate((x_star.reshape(h,w,-1)[...,b],Q.reshape(h,w,-1)[...,b],P.reshape(h,w,-1)[...,b],),axis=1)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--batch_size',type=int, default=100, help='Weight for the data term')
